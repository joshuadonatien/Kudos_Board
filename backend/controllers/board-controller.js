...
exports.getAllBoards = async(req, res) => {
    let boards = await prisma.board.findAll()
    res.json(boards)
}
...
exports.getBoardById = async (req, res) => {
    ...
    const board = await prisma.findUnique({where: {id}})
...
exports.deleteBoard = async (req,res) => {
    const id = Number(params.req.board_id)

    await prisma.board.delete({where:{order_id: id}})
    res.status(204).end()
}
